...
#from fixed_port_spiral import FixedPortSpiral
import joblib
import os
import numpy as np
...
class SpiralWgLossEbeam(PlaceAndAutoRoute):
    ...
    def _default_spiral(self):
        spiral = FixedLengthSpiralRounded(total_length=self.spiral_length, n_o_loops=self.n_o_loops,trace_template=self.trace_template)
        return spiral
    ...
    def _default_taper(self):
        taper = LinearWindowWaveguideTransition(start_trace_template=self.trace_template, end_trace_template=self.expanded_wg_template)
        return taper
    ...
    class Layout(PlaceAndAutoRoute.Layout):
        ...
        gap = i3.PositiveNumberProperty(doc="distance between two trenches", default=3.0)
        ...
        def _default_spiral(self):
            ...
            spiral_lo.set(incoupling_length=0, bend_radius=self.bend_radius, spacing=self._spacing_indesign, growth_direction="H", stub_direction="H")
            return spiral_lo

        def _default_wg(self):
            wg_lo = self.cell.wg.get_default_view(i3.LayoutView)
            wg_lo.set(shape=[(0.0,0.0), (self.spiral_width-self._spiral_in_out_distance(),0)])
            return wg_lo

        def _spiral_in_out_distance(self):
            return self.spiral.ports["out"].position.x - self.spiral.ports["in"].position.x
        ...
        def _default_child_transformations(self):
            ...
            trans["expanded_wg_right"] = i3.Translation((self.right_wg_length+self.taper_length,0))
            trans["wg_left"] = i3.HMirror(mirror_plane_x=0.0) + i3.Translation((-self._spiral_in_out_distance(),0))
            trans["taper_left"] = i3.HMirror(mirror_plane_x=0.0) +  i3.Translation((-self.spiral_width,0))
            trans["expanded_wg_left"] = i3.HMirror(mirror_plane_x=0.0) + i3.Translation((-self.spiral_width-self.taper_length,0))
            return trans

        def get_true_length(self):
            return self.spiral_width-self._spiral_in_out_distance()+self.right_wg_length+self.spiral_length

class SpiralWgLossEbeam2(PlaceAndAutoRoute):
    ## generate continue FBMS line combined with Class:SpiralWgLossEbeamFBMSMerged
    spiral = i3.ChildCellProperty()
    taper = i3.ChildCellProperty()
    wg = i3.ChildCellProperty()
    wg_right = i3.ChildCellProperty()
...
class SpiralWgLossEbeamFBMSMerged(i3.PCell):
    ## merge FBMS paths combined with Class:SpiralWgLossEbeam2
    wg_width = i3.PositiveNumberProperty(default=3.0)
    spiral_length = i3.PositiveNumberProperty(default=5000.0)

    class Layout(i3.LayoutView):

        def _generate_elements(self, elems):
            SpiralWg = SpiralWgLossEbeam2(spiral_length=self.spiral_length, wg_width=self.wg_width)
            lo = SpiralWg.Layout()
            shapes = lo.layout.flat_copy()
            paths = []
            for shape in shapes:
            ...
            elems += merged_paths
            elems += shapes
            print self.spiral_length, lo.get_true_length()
            return elems
...
# SpiralWg = SpiralWgLossEbeam(spiral_length=40000)
# lo = SpiralWg.Layout()
# gds_name = os.path.join(os.curdir, "spiral_wg.gds")
# # lo.write_gdsii(gds_name)
# shapes = lo.layout.flat_copy()
# flat_awg_lo = i3.LayoutCell().Layout(elements=lo.layout.flat_copy(),
#                                      ports=lo.ports)
# flat_awg_lo.write_gdsii(gds_name)
# paths = []
# for shape in shapes:
#     if shape.layer==i3.TECH.PPLAYER.WG.CLADDING:
#         paths.append(shape)
...
# cell = SpiralWgLossEbeamFBMSMerged(wg_width=3.0)
# # cell = SpiralWgLossEbeam2()
# lo = cell.Layout()
# gds_name = os.path.join(os.curdir, "spiral_wg.gds")
# lo.write_gdsii(gds_name)
