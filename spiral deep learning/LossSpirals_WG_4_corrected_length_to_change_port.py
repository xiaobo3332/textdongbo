...
class Spirals(PlaceAndAutoRoute):
    ...
    chip_length = i3.PositiveNumberProperty(default=3000.0, doc="Radius of curvature")
    ...
    couplingWG_l=i3.PositiveNumberProperty(default=10000.0, doc="Length of the coupling WG ")
    ...
    class Layout(PlaceAndAutoRoute.Layout):
        ...
        def _default_child_transformations(self):
            ...
            for counter, child  in enumerate(self.Spiral_list):
                ...
                if self.tipo==1:
                    ...
                    d['InPort' + str(counter)] = i3.HMirror()+ i3.Translation(translation=(0.0-(op[0]-ip[0])/2-self.tlport-a-b,#-self.chip_length/2-self.couplingWG_l,
                                                                                           self.n*counter*sx))
                    d['OutPort' + str(counter)] = i3.Translation(translation=(self.tlport+(op[0]-ip[0])/2+a+b,#self.chip_length/2,
                                                                              self.n*counter*sx))
                if self.tipo==2:
        ...
        def _generate_elements(self, elems):
            ...
            for counter, (child, length)  in enumerate(zip(self.Spiral_list, lengths)):
                ...
                total_length= length+self.R*(1+3.14159)-self.spacing+4*self.n_loops*self.spacing
            ...
            #i3.TECH.PPLAYER.NONE.LOGOTXT   when using isipp50g
                ...
                elems += i3.PolygonText(layer= i3.TECH.PPLAYER.WG.TEXT, text='Width = {} - Length = {} - R = {}'.format(width,
                                                                                                              total_length,
                                                                                                         self.R),
                                    ...
                                    coordinate=(-(op[0]-ip[0])/2-1000.0, counter*sx-100.0),
                                    alignment=(i3.TEXT_ALIGN_LEFT, i3.TEXT_ALIGN_LEFT),
                                    font = 2,
                                    height=20.0)                
            return elems
...
class Spirals_w(PlaceComponents):
    ...
    def _default_Spirals_w_list(self):
        ...
        for w, width in enumerate(self.width_vec):
            ...
            Spirals_w_list.append(Spirals(s_length_vec=self.s_length_vec, waveguide_template=wg_t, tipo=self.tipo, width=w+1, n_loops=self.n,
...
#def get_number_loops(spiral):
    #length = spiral.n_loops
    #'The number of loops is : ', length
    #return length
...
def get_lengths(spirals):
    ...
    # which are the wide waveguides to be cleaved.      
            ...
            length_total = (
                ...
                #+link2.trace.bend_radius*(1+3.14159)-2*100+4*get_number_loops(spiral)*100
...
#cell=Spirals_w(s_length_vec=[14800, 20000.0,30000.0],width_vec=[3.3], tipo=1)
#cell=Spirals_w(s_length_vec=[14725],width_vec=[4.5], tipo=1, n=1)
#cell=Spirals_w(s_length_vec=[31465],width_vec=[4.5], tipo=1, n=2)
#cell=Spirals_w(s_length_vec=[51405],width_vec=[4.5], tipo=1, n=3)
cell=Spirals_w(s_length_vec=[74544],width_vec=[5.0], tipo=1, n=4)


#cell=Spirals_w(s_length_vec=[5000.0, 5000.0,5000.0,5000.0],width_vec=[3.2,3.4,3.6,3.8,4.0], tipo=2)

lv = cell.Layout()
lv.write_gdsii("Loss Spirals w_5C0p_w50_L2000 nloops 4 15um taper.gds")
