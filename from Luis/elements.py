...
def grating(wg, l, p, dc, pos=0):
	# wg [wg] -> waveguide to place the grating
	...
	circuit_layout = circuit.Layout(
		child_transformations={
			...
			"grating2": i3.Rotation(rotation_center=(0.0,0.0),
						...
						absolute_rotation=False)+
				i3.Translation((
				gsl*3/2+2*tl+i_s[0]+(4*nl+1)*s+2*il, 0))})
	...
	circuit_layout = circuit.Layout(
		child_transformations={
			...
			"grating4": i3.Rotation(rotation_center=(0.0,0.0),
								rotation=180,
							absolute_rotation=False)+
			i3.Translation((gsl*3/2.0+2*tl+40, 2*radius+gap+gap1+wg_ring_width+wg_coupler_width))})
